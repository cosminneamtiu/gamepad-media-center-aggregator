...
private:
    ...
    /// Live P2P buffering overlay (torrent sources only). A non-focusable pill
    /// (spinner + one live label) built lazily on the first torrent playback and
    /// laid on top of the VideoView, driven by a periodic timer that samples
    /// torrent::EngineSession::stats() (peers / ⬇ speed / buffered %). It is torn
    /// down as soon as playback really starts (first LOADING_END / progress) or the
    /// player goes away. Never disturbs OSD navigation. All of this compiles only
    /// when ENABLE_TORRENT is defined — an OFF build is byte-for-byte unchanged.
    void showTorrentOverlay();    ///< create if needed, reset to "connecting", start the ticker
    void updateTorrentOverlay();  ///< one sample of stats() -> label text (ticker callback)
    void hideTorrentOverlay();    ///< stop the ticker and hide the pill (idempotent)

    brls::Box* torrentOverlay = nullptr;
    brls::Label* torrentOverlayLabel = nullptr;
